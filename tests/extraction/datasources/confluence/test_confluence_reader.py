...
class Arrangements:
    ...
    def __init__(self, fixtures: Fixtures) -> None:
        ...
        self.service = ConfluenceDatasourceReader(
...
class Assertions:
    ...
    def assert_confluence_pages(self, confluence_pages: List[dict]) -> None:
        ...
        if self.fixtures.export_limit is not None:
            assert len(confluence_pages) <= self.fixtures.export_limit
        else:
            assert len(confluence_pages) == len(all_available_pages)
...
class Manager:
    ...
    def get_service(self) -> ConfluenceDatasourceReader:
...
class TestConfluenceReader:
    ...
    async def test(
        ...
        # Act
        confluence_pages = []
        async for confluence_page in service.read_all_async():
            confluence_pages.append(confluence_page)

        # Assert
        manager.assertions.assert_confluence_pages(confluence_pages)
